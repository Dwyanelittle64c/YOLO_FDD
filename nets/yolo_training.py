...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, input_shape, anchors_mask = [[6,7,8], [3,4,5], [0,1,2]], label_smoothing = 0,IoU='CIoU',sod=False):
        ...
        if sod:
            self.stride         = [32, 16, 8, 4]
            self.balance        = [0.4, 1.0, 4, 10]
        else:
            self.stride         = [32, 16, 8]
    ...
    def bbox_iou(self,box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, eps=1e-7,IoU='CIoU'):
        if IoU=='GIoU':
            GIoU=True
        elif IoU=='DIoU':
            DIoU=True
    ...
    def __call__(self, predictions, targets, imgs): 
        ...
        for i, prediction in enumerate(predictions): 
            ...
            if n:
                ...
                #   计算预测框和真实框的回归损失
                #-------------------------------------------#
                # if selected_tbox.shape[0]==1:
                #     print(box.T,selected_tbox)
                iou                     = self.bbox_iou(box.T, selected_tbox, x1y1x2y2=False, IoU=self.IoU)
