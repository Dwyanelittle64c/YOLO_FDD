...
if __name__ == "__main__":
    ...
    # yolov7的构型，当且仅当backbone=0时有效
    phi = 'l'

    # 是否使用第四个检测头(当且仅当其他参数默认，且还没改好，不建议使用)
    sod = False
    ...
    #   创建yolo模型
    # ------------------------------------------------------#
    model = YoloBody(anchors_mask, num_classes, phi, pretrained=pretrained, phi_attention=phi_attention, DCN_fpn=DCN_fpn,
                     backbone=backbone, focus=focus, sod=sod, head=head, DCN_backbone=DCN_backbone,
                     MPCA_backbone=MPCA_backbone, MPCA_fpn=MPCA_fpn, MPCBAM_backbone=MPCBAM_backbone,MPCBAM_fpn=MPCBAM_fpn,
                     BiFPN=BiFPN, SE_concat=SE_concat, DW=DW,SA_concat=SA_concat)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        # ------------------------------------------------------#
    ...
    #   获得损失函数
    # ----------------------#
    yolo_loss = YOLOLoss(anchors, num_classes, input_shape, anchors_mask, label_smoothing, IoU=IoU,sod=sod)
    # ----------------------#
    #   记录Loss
    # ----------------------#
    if local_rank == 0:
    ...
    if True:
        ...
        if Freeze_Train:
            ...
            # for param in model.backbone.dark21.parameters():
            #     param.requires_grad=False

            # for param in model.backbone.stem.parameters():
            #     param.requires_grad = True
            # for param in model.yolo_head_P3.parameters():
            #     param.requires_grad = True
            # for param in model.yolo_head_P4.parameters():
            #     param.requires_grad = True
            # for param in model.yolo_head_P5.parameters():
            #     param.requires_grad = True
