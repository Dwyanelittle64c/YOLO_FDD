...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        temp_map = eval_callback.on_epoch_end(epoch + 1, model_train_eval)
        if temp_map is not None:
            loss_history.append_map(temp_map)
        ...
        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            ...
            torch.save(save_state_dict, os.path.join(save_dir, "best_loss_weights.pth"))

        if len(loss_history.val_map) <= 1 or temp_map is not None and temp_map >= max(loss_history.val_map):
